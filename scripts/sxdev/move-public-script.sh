...
make_temp_file() {
  if have_cmd mktemp; then
    mktemp "${TMPDIR:-/tmp}/sx-ctl-move-public.XXXXXX"
  else
    tmp="${TMPDIR:-/tmp}/sx-ctl-move-public.$$"
    : >"$tmp" || return 1
    printf '%s\n' "$tmp"
  fi
}
...
ensure_public_root() {
  ...
  [ -d "$PUBLIC_SCRIPTS_DIR" ] || die "Public scripts dir not found: $PUBLIC_SCRIPTS_DIR"
}
...
main() {
  ...
  */* | *..*) die "Invalid file name: $new_file_name" ;;
  ...
  new_id="$new_category.$base_name"
  new_path="scripts/$new_category/$new_file_name"
  new_file="$PUBLIC_ROOT/$new_path"
  ...
  mkdir -p "$PUBLIC_ROOT/scripts/$new_category"
  ...
  awk -F '|' -v OFS='|' \
    ...
    NR == 1 { print; next }
    $1 == old_id {
      print new_id, "public", new_category, label, new_path, description, shell_name, deps, risk
      next
    }
    { print }
