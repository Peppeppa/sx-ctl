...
ensure_public_root() {
  if [ ! -d "$PUBLIC_ROOT" ]; then
    die "Public root not found: $PUBLIC_ROOT"
  fi

  if [ ! -f "$PUBLIC_ROOT/sx-ctl.sh" ] || [ ! -f "$PUBLIC_ROOT/manifest.txt" ] || [ ! -d "$PUBLIC_ROOT/scripts" ]; then
    die "Public root is not an sx-ctl repository. Set SX_PUBLIC_ROOT=/path/to/sx-ctl or run from the repo root."
  fi
}
